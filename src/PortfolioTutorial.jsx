...
function AssetTypesPill() {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', justifyContent: 'center' }}>
      {types.map(t => (
        <div key={t.label} style={{ padding: '6px 14px', background: t.bg, border: `1px solid ${t.color}`, borderRadius: '20px', fontFamily: 'var(--font-body)', fontSize: '12px', color: t.color, fontWeight: 700, letterSpacing: '0.06em' }}>
          {t.label}
        </div>
      ))}
    </div>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9990, background: 'rgba(0,0,0,0.88)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--bd)', borderRadius: '16px', padding: '28px 24px', maxWidth: '360px', width: '100%', position: 'relative', boxShadow: '0 0 40px rgba(55,138,221,0.12)' }}>
        ...
        {!current.final && (
          <button onClick={dismiss} style={{ position: 'absolute', top: '16px', right: '16px', background: 'transparent', border: 'none', color: 'var(--t6)', fontFamily: 'var(--font-body)', fontSize: '12px', cursor: 'pointer', letterSpacing: '0.04em' }}>
            {p.tutSkip}
          </button>
        )}
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          {step > 0 && (
            <button onClick={() => setStep(s => s - 1)} style={{ flex: 1, padding: '12px', background: 'transparent', border: '1px solid var(--bd2)', borderRadius: '8px', color: 'var(--t5)', fontFamily: 'var(--font-body)', fontSize: '12px', fontWeight: 700, cursor: 'pointer', letterSpacing: '0.04em' }}>
              {p.tutPrev}
            </button>
          )}
          <button
            onClick={() => step < STEPS.length - 1 ? setStep(s => s + 1) : dismiss()}
            style={{ flex: 1, padding: '12px', background: current.final ? 'rgba(55,138,221,0.15)' : 'rgba(55,138,221,0.08)', border: '1px solid #378ADD', borderRadius: '8px', color: '#378ADD', fontFamily: 'var(--font-body)', fontSize: '12px', fontWeight: 700, cursor: 'pointer', letterSpacing: '0.04em' }}
          >

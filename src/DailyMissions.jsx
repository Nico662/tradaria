...
export default function DailyMissions() {
  ...
  return (
    <div style={{ margin: '0 20px 12px', background: 'var(--bg-card)', border: '1px solid var(--bd)', borderRadius: '8px', padding: '10px 14px' }}>
      <div style={{ fontSize: '12px', color: 'var(--bd2)', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: '8px', fontFamily: 'var(--font-body)' }}>
        {label}
      </div>
        ...
        {missions.map(m => {
          ...
          return (
            <div key={m.id}>
              ...
              <div style={{ fontSize: '12px', color: '#3a4a5c', paddingLeft: '11px', marginBottom: '5px', lineHeight: 1.4 }}>
                {desc}
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                <span style={{ fontFamily: 'var(--font-body)', fontSize: '12px', color: 'var(--bd2)', flexShrink: 0 }}>
                  {progress}/{m.target}
                </span>
              </div>
            </div>

  ...
  if (screen === 'async_accept' && asyncDuelData) return (
    <div id="gtm-root" style={{ position: 'relative' }}>
      ...
      <div style={{ padding: '40px 28px', position: 'relative', zIndex: 2, textAlign: 'center' }}>
        ...
        <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '32px', letterSpacing: '0.04em' }}>
          {t.arena.asyncCanYouBeat}
        </div>
        <div style={{ marginBottom: '20px' }}>
          <div style={{ fontSize: '12px', color: 'var(--text-muted)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '8px' }}>{t.arena.yourName}</div>
      ...
      <div id="gtm-root" style={{ position: 'relative' }}>
        ...
        <div style={{ padding: '40px 28px', position: 'relative', zIndex: 2, textAlign: 'center' }}>
          ...
          <div style={{ padding: '14px', background: 'var(--bg-surface)', border: '1px solid var(--border-default)', borderRadius: '8px', marginBottom: '16px' }}>
            <div style={{ fontFamily: 'var(--font-body)', fontSize: '12px', color: 'var(--green)', wordBreak: 'break-all', marginBottom: '10px' }}>{link}</div>
            <button onClick={handleCopy}
              style={{ width: '100%', padding: '10px', background: 'var(--green-dim)', border: '1px solid var(--border-green)', borderRadius: '6px', color: 'var(--green)', fontFamily: 'var(--font-body)', fontSize: '12px', fontWeight: 700, cursor: 'pointer', letterSpacing: '0.06em' }}>
        ...
        <div style={{ padding: '40px 28px', position: 'relative', zIndex: 2, textAlign: 'center' }}>
          ...
          <div style={{ padding: '16px', background: 'var(--bg-surface)', border: '1px solid var(--bd2)', borderRadius: '8px', marginBottom: '20px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={handleCopy}
                style={{ flex: 1, padding: '10px', background: 'var(--green-dim)', border: '1px solid var(--border-green)', borderRadius: '6px', color: 'var(--green)', fontFamily: 'var(--font-body)', fontSize: '12px', fontWeight: 700, cursor: 'pointer', letterSpacing: '0.06em' }}>
                {status === t.arena.asyncCopied ? status : t.arena.asyncCopyLink}
              </button>
              <button onClick={handleShare}
                style={{ flex: 1, padding: '10px', background: 'rgba(232,184,75,0.08)', border: '1px solid var(--color-neutral)', borderRadius: '6px', color: 'var(--color-neutral)', fontFamily: 'var(--font-body)', fontSize: '12px', fontWeight: 700, cursor: 'pointer', letterSpacing: '0.06em' }}>
                {t.arena.asyncShare}
              </button>
            </div>
          </div>
          <div style={{ fontSize: '12px', color: 'var(--text-muted)', letterSpacing: '0.06em' }}>{t.arena.asyncRivalTime}</div>
        </div>
      </div>
    );
  }
      ...
      <div id="gtm-root" style={{ position: 'relative' }}>
        ...
        <div style={{ padding: '40px 28px 36px', position: 'relative', zIndex: 2 }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: '24px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', marginBottom: '8px' }}>
              <div style={{ flex: 1, padding: '16px', background: chalWon ? 'rgba(0,229,160,0.08)' : 'var(--bg-surface)', border: `1px solid ${chalWon ? 'var(--border-green)' : 'var(--border-default)'}`, borderRadius: '10px' }}>
                {chalWon && !isDraw && <div style={{ fontSize: '12px', color: 'var(--green)', letterSpacing: '0.1em', marginBottom: '4px' }}>{t.arena.asyncWinner}</div>}
                <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.06em', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{chal.name || '—'}</div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 6px' }}>
                ...
                {isDraw && <span style={{ fontSize: '12px', color: 'var(--color-neutral)', marginTop: '4px' }}>{t.arena.asyncDraw}</span>}
              </div>
              <div style={{ flex: 1, padding: '16px', background: !chalWon && !isDraw ? 'rgba(255,126,179,0.08)' : 'var(--bg-surface)', border: `1px solid ${!chalWon && !isDraw ? 'var(--color-down)' : 'var(--border-default)'}`, borderRadius: '10px' }}>
                {!chalWon && !isDraw && <div style={{ fontSize: '12px', color: 'var(--color-down)', letterSpacing: '0.1em', marginBottom: '4px' }}>{t.arena.asyncWinner}</div>}
                <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.06em', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{riv.name || '—'}</div>
          ...
          {chal.answers?.length > 0 && (
            <div style={{ marginBottom: '20px' }}>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '10px', fontFamily: 'var(--font-body)' }}>{t.arena.asyncRoundTable}</div>
              <div style={{ display: 'grid', gridTemplateColumns: '28px 1fr 72px 72px', gap: '4px 8px', fontSize: '12px', color: 'var(--text-muted)', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: '6px', padding: '0 2px' }}>
              ...
              {chal.answers.map((ca, i) => {
                ...
                return (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '28px 1fr 72px 72px', gap: '4px 8px', padding: '7px 2px', borderBottom: '1px solid var(--border-default)', alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: '12px', color: 'var(--text-secondary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{chart?.asset || '—'}</span>
                    <div style={{ textAlign: 'center', fontSize: '12px', color: ca.win ? 'var(--green)' : 'var(--color-down)', fontFamily: 'var(--font-body)' }}>
                      {ca.choice?.toUpperCase().slice(0, 1)} {ca.win ? '✓' : '✗'}
                    </div>
                    <div style={{ textAlign: 'center', fontSize: '12px', color: ra ? (ra.win ? 'var(--green)' : 'var(--color-down)') : 'var(--text-muted)', fontFamily: 'var(--font-body)' }}>
          ...
          <div id="async-share-card" style={{ position: 'absolute', left: '-9999px', top: 0, width: '320px', background: 'var(--bg-base)', border: '1px solid var(--color-neutral)', borderRadius: '12px', padding: '28px 24px', fontFamily: 'var(--font-body)' }}>
            ...
            <div style={{ display: 'flex', gap: '16px', marginBottom: '16px' }}>
              <div style={{ flex: 1, textAlign: 'center' }}>
                ...
                <div style={{ fontSize: '12px', color: '#5a6a7d', marginTop: '2px' }}>{chal.name}</div>
              </div>
              <div style={{ alignSelf: 'center', color: '#5a6a7d' }}>vs</div>
              <div style={{ flex: 1, textAlign: 'center' }}>
                <div style={{ fontFamily: 'var(--font-body)', fontWeight: 800, fontSize: '36px', color: 'var(--color-down)' }}>{rivCorrect}</div>
                <div style={{ fontSize: '12px', color: '#5a6a7d', marginTop: '2px' }}>{riv.name}</div>
              </div>
            </div>
            <div style={{ fontSize: '12px', color: 'var(--color-neutral)', letterSpacing: '0.1em' }}>tradiko.dev</div>
          </div>
        </div>
  ...
  if (screen === 'lobby') return (
    <div id="gtm-root" style={{ position: 'relative' }}>
      ...
      <div style={{ padding: '40px 28px', position: 'relative', zIndex: 2 }}>
        <button onClick={goBack}
          style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', fontFamily: 'var(--font-body)', fontSize: '12px', cursor: 'pointer', letterSpacing: '0.06em', marginBottom: '32px', display: 'block' }}
        ...
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          ...
          <div style={{ fontSize: '12px', color: '#5a6a7d', letterSpacing: '0.15em', textTransform: 'uppercase' }}>{t.arena.sub}</div>
        </div>
        ...
        {status && <div style={{ fontSize: '12px', color: 'var(--color-down)', marginBottom: '12px' }}>{status}</div>}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            ...
            <button onClick={joinRoom} disabled={!name.trim() || joinCode.length < 4}
              style={{ padding: '12px 20px', background: name.trim() && joinCode.length >= 4 ? 'rgba(232,184,75,0.08)' : 'var(--bg-surface)', border: `1px solid ${name.trim() && joinCode.length >= 4 ? 'var(--color-neutral)' : 'var(--bd2)'}`, borderRadius: '6px', color: name.trim() && joinCode.length >= 4 ? 'var(--color-neutral)' : 'var(--text-muted)', fontFamily: 'var(--font-body)', fontSize: '12px', fontWeight: 700, cursor: name.trim() && joinCode.length >= 4 ? 'pointer' : 'not-allowed', letterSpacing: '0.06em', whiteSpace: 'nowrap' }}>
              {t.arena.joinRoom}
            </button>
          </div>
        </div>
        ...
        {myDuels !== null && (
          <div style={{ marginTop: '24px' }}>
            <div style={{ fontSize: '12px', color: 'var(--text-muted)', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '10px', fontFamily: 'var(--font-body)' }}>{t.arena.myDuelsTitle}</div>
            {myDuels.length === 0 ? (
              <div style={{ fontSize: '12px', color: 'var(--text-muted)', fontFamily: 'var(--font-body)', padding: '12px 0' }}>{t.arena.myDuelsEmpty}</div>
            ) : myDuels.map(d => {
              ...
              return (
                ...
                >
                  <div>
                    <div style={{ fontFamily: 'var(--font-body)', fontWeight: 700, fontSize: '12px', color: 'var(--text-primary)', letterSpacing: '0.12em', marginBottom: '3px' }}>{d.code}</div>
                    <div style={{ fontSize: '12px', color: statusColor, fontFamily: 'var(--font-body)' }}>{statusLabel}</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    {isCompleted && (
                      <div style={{ fontFamily: 'var(--font-body)', fontWeight: 800, fontSize: '13px', color: won ? 'var(--green)' : lost ? 'var(--color-down)' : 'var(--color-neutral)', marginBottom: '2px' }}>
                        {d.myCorrect} – {d.oppCorrect}
                      </div>
                    )}
                    {isCompleted && (
                      <div style={{ fontSize: '12px', color: won ? 'var(--green)' : lost ? 'var(--color-down)' : 'var(--color-neutral)', fontFamily: 'var(--font-body)' }}>
                        {won ? t.arena.myDuelsWon : lost ? t.arena.myDuelsLost : t.arena.myDuelsDraw}
                      </div>
                    )}
                    {isWaiting && d.status === 'waiting_rival' && (
                      <div style={{ fontSize: '12px', color: 'var(--color-neutral)', fontFamily: 'var(--font-body)' }}>
                        {t.arena.myDuelsExpiresIn} {d.hoursLeft}h
                      </div>
                    )}
                    {isExpired && (
                      <div style={{ fontSize: '12px', color: 'var(--text-muted)', fontFamily: 'var(--font-body)' }}>—</div>
                    )}
                  </div>
                </button>
              );
        ...
        <div style={{ marginTop: '20px', padding: '16px', background: 'var(--bg-surface)', border: '1px solid var(--border-default)', borderRadius: '8px' }}>
          <div style={{ fontSize: '12px', color: 'var(--text-muted)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '10px' }}>{t.arena.howTitle}</div>
    ...
    <div id="gtm-root" style={{ position: 'relative' }}>
      ...
      <div style={{ padding: '40px 28px', position: 'relative', zIndex: 2, textAlign: 'center' }}>
        ...
        {status === 'JOINING_CHALLENGE' && (
          <div style={{ marginBottom: '24px', fontSize: '12px', color: 'var(--text-muted)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
        ...
        {status === 'waiting_for_friend' && roomCode ? (
          <div style={{ marginBottom: '24px' }}>
            <div style={{ fontSize: '12px', color: 'var(--text-muted)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '12px' }}>{t.arena.shareCode}</div>
            ...
            <div style={{ marginTop: '12px', fontSize: '12px', color: 'var(--text-muted)', letterSpacing: '0.06em' }}>{t.arena.waitingFriend}</div>
          </div>
        ) : status !== 'JOINING_CHALLENGE' ? (
          <>
            <div style={{ fontSize: '12px', color: 'var(--text-muted)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '32px' }}>
              {t.arena.playingAs} <span style={{ color: getUsernameColor(activeCosmetics) || 'var(--green)' }}>{name}</span>{user?.battlePassMechanics?.includes('mechanic_verified_badge') && <VerifiedBadge size={11} />}
            </div>
        ...
        <button onClick={goBack}
          style={{ background: 'transparent', border: '1px solid var(--bd2)', borderRadius: '6px', color: 'var(--text-muted)', fontFamily: 'var(--font-body)', fontSize: '12px', padding: '8px 16px', cursor: 'pointer', letterSpacing: '0.06em' }}>
          {t.arena.cancel}
        </button>
      </div>
    </div>
      ...
      <div id="gtm-root" style={{ position: 'relative' }}>
        ...
        <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--border-default)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'relative', zIndex: 2 }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px', minWidth: '70px' }}>
            ...
            <div style={{ fontSize: '12px', color: 'var(--text-muted)', letterSpacing: '0.06em' }}>{name.toUpperCase()}</div>
            <button onClick={goBack}
              style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', fontFamily: 'var(--font-body)', fontSize: '12px', cursor: 'pointer', letterSpacing: '0.06em', padding: '2px 0', marginTop: '2px', minHeight: '24px', minWidth: '44px' }}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px' }}>
            <div style={{ fontSize: '12px', color: 'var(--text-muted)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>{t.game.round.toLowerCase()} {round}/{total}</div>
       ...
       <div style={{ padding: '8px 16px', position: 'relative', zIndex: 2, display: 'flex', gap: '8px' }}>
        <div style={{ flex: 1, background: 'var(--bg-surface)', border: '1px solid var(--border-default)', borderRadius: '6px', padding: '8px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: '12px', color: 'var(--text-muted)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>{name}</span>
        ...
        {phase === 'waiting_opponent' && (
          <div style={{ padding: '16px 20px', textAlign: 'center', fontSize: '12px', color: 'var(--text-muted)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
          ...
          <div style={{ padding: '0 20px 16px', position: 'relative', zIndex: 2 }}>
            <div style={{ borderRadius: '8px', padding: '14px 16px', border: `1px solid ${myResult?.win ? 'var(--border-green)' : 'var(--color-down)'}`, background: myResult?.win ? 'rgba(0,229,160,0.05)' : 'rgba(255,126,179,0.05)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                ...
                <div style={{ fontFamily: 'var(--font-body)', fontWeight: 800, fontSize: '20px', color: myResult?.win ? 'var(--green)' : 'var(--color-down)' }}>
                  {myResult?.win ? '+100' : '±0'}
                </div>
              </div>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)', letterSpacing: '0.06em' }}>
              ...
              {!isBotGame && !isAsyncGame && (
                <div style={{ marginTop: '8px', fontSize: '12px', color: 'var(--text-muted)', textAlign: 'center', letterSpacing: '0.06em' }}>
                  {t.arena.nextRound}
                </div>
              )}
              {isAsyncGame && (
                <button onClick={nextAsyncRound}
                  style={{ marginTop: '10px', width: '100%', padding: '10px', background: 'var(--green-dim)', border: '1px solid var(--border-green)', borderRadius: '6px', color: 'var(--green)', fontFamily: 'var(--font-body)', fontSize: '12px', fontWeight: 700, letterSpacing: '0.06em', cursor: 'pointer' }}>
        ...
        {!isBotGame && chatMsg && (
          <div style={{ position: 'absolute', top: '60px', right: '16px', zIndex: 20, background: 'var(--bg-elevated)', border: '1px solid var(--bd2)', borderRadius: '8px', padding: '8px 12px', fontFamily: 'var(--font-body)', fontSize: '12px', color: 'var(--text-primary)', maxWidth: '160px' }}>
            <span style={{ color: 'var(--green)', fontSize: '12px' }}>{chatMsg.from}</span>
            <div>{chatMsg.msg}</div>
          </div>
        )}
      ...
      return (
        <div id="gtm-root" style={{ position: 'relative' }}>
          ...
          <div style={{ padding: '40px 28px 36px', position: 'relative', zIndex: 2, textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '12px', color: 'var(--text-muted)', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '32px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => { setScreen('lobby'); setResult(null); setScores({}); setFinalData(null); resetBotState(); }}
                style={{ flex: 1, padding: '14px', background: 'var(--bg-surface)', border: '1px solid var(--border-green)', borderRadius: '6px', color: 'var(--green)', fontFamily: 'var(--font-body)', fontSize: '12px', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', cursor: 'pointer' }}>
                {t.arena.rematch}
              </button>
              <button onClick={goBack}
                style={{ flex: 1, padding: '14px', background: 'var(--bg-surface)', border: '1px solid var(--bd2)', borderRadius: '6px', color: 'var(--text-secondary)', fontFamily: 'var(--font-body)', fontSize: '12px', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', cursor: 'pointer' }}>
                {t.arena.menu}
              </button>
            </div>
          </div>
      ...
      <div id="gtm-root" style={{ position: 'relative' }}>
        ...
        <div style={{ padding: '40px 28px 36px', position: 'relative', zIndex: 2, textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', marginBottom: '32px' }}>
            <div style={{ flex: 1, background: 'var(--bg-surface)', border: '1px solid var(--border-green)', borderRadius: '10px', padding: '20px' }}>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '8px' }}>{name}</div>
          ...
          {!isBotGame && rematchState === 'requested' && (
            <div style={{ marginBottom: '10px' }}>
              <div style={{ fontSize: '12px', color: 'var(--color-neutral)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '12px' }}>
          ...
          {!isBotGame && rematchState === 'countdown' && (
            <div style={{ padding: '20px', background: 'var(--bg-surface)', border: '1px solid var(--border-green)', borderRadius: '8px', marginBottom: '10px' }}>
              <div style={{ fontSize: '12px', color: 'var(--green)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '8px' }}>
                {t.arena.rematchStarting}
              </div>
              <div style={{ fontFamily: 'var(--font-body)', fontWeight: 800, fontSize: '48px', color: 'var(--green)' }}>
                {rematchCountdown}
              </div>
            </div>
          )}
          ...
          {personalStats && (
            <div style={{ marginTop: '4px', padding: '12px 16px', background: 'var(--bg-page)', border: '1px solid var(--border-default)', borderRadius: '8px' }}>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '8px', fontFamily: 'var(--font-body)' }}>{t.stats.myHistory}</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
                {[
                  ...
                  <div key={s.label} style={{ textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: '12px', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginTop: '2px' }}>{s.label}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div id="share-card-arena" style={{ position: 'absolute', left: '-9999px', top: 0, width: '320px', background: 'var(--bg-page)', border: `1px solid ${iWon ? 'var(--border-green)' : isDraw ? 'var(--color-neutral)' : 'var(--color-down)'}`, borderRadius: '12px', padding: '28px 24px', fontFamily: 'var(--font-body)' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', marginBottom: '16px' }}>
            <div style={{ flex: 1, textAlign: 'center' }}>
              <div style={{ fontFamily: 'var(--font-body)', fontWeight: 800, fontSize: '32px', color: 'var(--green)' }}>{myScore}</div>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '2px' }}>{name}</div>
            </div>
            <div style={{ alignSelf: 'center', fontSize: '14px', color: 'var(--text-muted)' }}>vs</div>
            <div style={{ flex: 1, textAlign: 'center' }}>
              <div style={{ fontFamily: 'var(--font-body)', fontWeight: 800, fontSize: '32px', color: 'var(--color-down)' }}>{oppScore}</div>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '2px' }}>{opponent}</div>
            </div>
          </div>
